...
public class EarthAtmosphereRenderer {
    ...
    private static final String FRAG_SRC =
            "#version 150 core\n" +
                    ...
                    "    vec3 sunColor = vec3(1.00, 0.95, 0.80);\n" +
                    "    // Blue earth colors\n" +
                    "    vec3 innerColor = vec3(0.20, 0.50, 1.00);\n" +
                    "    vec3 outerColor = vec3(0.05, 0.20, 0.85);\n" +
                    "    vec3 backColor = vec3(0.01, 0.05, 0.20);\n" +
    ...
    private static void initIfNeeded() {
        ...
        if (GL20.glGetProgrami(shaderProgram, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            throw new RuntimeException("[EarthAtmosphere] Link error: " + GL20.glGetProgramInfoLog(shaderProgram));
        }
    ...
    private static int compileShader(int type, String src) {
        ...
        if (GL20.glGetShaderi(id, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            throw new RuntimeException("[EarthAtmosphere] Compile error:\n" + GL20.glGetShaderInfoLog(id));
        }
        return id;
    }
